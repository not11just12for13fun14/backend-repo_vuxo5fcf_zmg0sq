...
def list_products(category: Optional[str] = None, curated: Optional[bool] = None):
    query = {}
    if category:
    ...
    try:
        ...
        # Normalize _id for Pydantic (remove Mongo _id)
        for d in docs:
            d.pop("_id", None)
        return docs
    except Exception as e:
...
def list_services(category: Optional[str] = None, curated: Optional[bool] = None):
    query = {}
    if category:
        query["category"] = category
    if curated is not None:
...
def list_gigs(category: Optional[str] = None, remote: Optional[bool] = None):
    query = {}
    if category:
        query["category"] = category
